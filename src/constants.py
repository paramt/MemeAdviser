class Thresholds:
	# The minimum time between recommendations (in seconds)
	spam_time = 18000

	# Only comment on submission if number of upvotes is less than this
	comment = 800

	# Only send PMs if number of upvotes is less than this
	pm = 800


class Messages:
	comment = "I **strongly advise** investing! This meme hit #1 on [hot](https://www.reddit.com/r/memeeconomy/hot/) within **{time}**, at **{upvotes}** upvotes. If you invest now, you'll break even at **{break_even}** upvotes.\n\n[Click here](https://www.param.me/meme/calculator/break-even) to calculate the current break-even point. [Click here](https://www.reddit.com/message/compose?to=MemeAdviser&subject=Subscribe&message=Subscribe) to subscribe to daily market updates.\n***\n^(I'm a bot | [contact](https://www.reddit.com/message/compose?to=hypnotic-hippo&subject=MemeAdviser) | [source](https://github.com/paramt/MemeAdviser) | [website](https://www.param.me/MemeAdviser))"
	pm = "[This meme](https://reddit.com{link}) just hit #1 on MemeEconomy with only {upvotes} upvotes! Invest now and break even at {break_even} upvotes\n***\n^(You're recieving this message because you've subscribed to this bot. To unsubscribe, reply 'Unsubscribe')"


LOGFILE = "../MemeAdviser.wiki/memeadviser.log"
